...
function preload() {
  ...
  //preloading the image of a collided monkey
  monkeyCollidedImage = loadImage("Monkey_01.png");
...
function setup() {
  ...
  monkey.addAnimation("monkey_collided", monkeyCollidedImage);
  monkey.scale = 0.1;
  //monkey.velocityX = 3;
  ...
  gameOver = createSprite(350, 250, 15, 15);
...
function draw() {
  ...
  if (gameState === "play"){
    ...
    //setting velocities of game objects to 0
    backGround.velocityX = 0;
    monkey.velocityY = 0;
    bananaGroup.setVelocityXEach(0);
    stoneGroup.setVelocityXEach(0);

    //setting lifetimes of game objects so they are not destoyed
    bananaGroup.setLifetimeEach(-1);
    stoneGroup.setLifetimeEach(-1);

    //changing the monkey's animation
    monkey.changeAnimation("monkey_collided", monkeyCollidedImage);

    //displaying the game over label
  ...
  textSize(18);
  fill("white");
  text("Score: " + count, 500 , 110);
  text("*While dodging obstacles jump and grab hold of the bananas using the SPACE key*", 15, 440);
